...
import { IPROXY_UPDATE_DIR, IPROXY_UPDATE_CONFIG, SYSTEM_IS_MACOS, APP_VERSION } from './const';
...
const LAUNCHER_VERSION_FILE = path.join(IPROXY_UPDATE_DIR, 'launcher_version');
    ...
    try {
        const url =
            updateChannel === 'stable'
                ? 'https://gw.alipayobjects.com/os/iProxy/release.json'
                : 'https://gw.alipayobjects.com/os/iProxy/beta-release.json';
        ...
        if (compareVersions(data.version, version) === 1) {
            ...
            try {
                fs.removeSync(IPROXY_UPDATE_DIR);
            } catch (e) {}
            fs.mkdirpSync(IPROXY_UPDATE_DIR);
